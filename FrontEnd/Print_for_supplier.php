...
?>
	...
	<?php
		$query=mysqli_query($conn,"select Product.Name,Product.Brand,Product.Size,supp_list.SCost,supp_list.Quantity from Product inner join supp_list on Product.PID = supp_list.PID where supp_list.SID=(select max(SID) from supp_list)");
		...
		$query1=mysqli_query($conn,"select * from Account where TID = (select MAX(SID) from Supplier)");
	...
	<span>
		<button style="margin-left:30px;"  type="submit" onclick = "location.href = 'Manager.php'" class ="btn btn-primary btn-lg" >Finish</button><br/>
	</span>
